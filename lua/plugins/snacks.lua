return {
	...
	opts = {
		...
		win = {
			input = {
			  keys = {
			    -- Custom mnemonic hotkeys
			    ["v"] = { "edit_vsplit", mode = { "n" } },
			    ["h"] = { "edit_split", mode = { "n" } },
			    ["t"] = { "edit_tab", mode = { "n" } },
			    -- Optional: Map 'esc' to close in normal mode
			    ["<Esc>"] = { "close", mode = { "n" } },
			  },
			},
		      },
  ...
  -- stylua: ignore start
	keys = {
		...
		{ "<leader>fe", function() Snacks.picker.files({ cwd = vim.fn.stdpath("config") }) end, desc = "Find config file" },
